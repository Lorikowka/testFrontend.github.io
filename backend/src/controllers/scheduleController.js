...
const buildSlotUsageMap = async ({ excludePaymentId = null } = {}) => {
  ...
  for (const session of sessions) {
    ...
    addEntry(getDateFromSessionDatetime(session.session_datetime) || session.session_date, getTimeFromSessionDatetime(session.session_datetime) || session.session_time, {
      source: 'session', paymentId: session.payment_id, serviceId: service.id, type: service.type, email: session.client_email, phone: session.client_phone
    });
  }
  ...
  for (const payment of reservedPayments) {
    ...
    addEntry(reserved.sessionDate, reserved.sessionTime, {
      source: 'payment', paymentId: payment.id, serviceId: service.id, type: service.type,
  ...
  try {
    ...
    for (let i = 1; i <= safeDays; i++) {
      ...
      for (const time of allTimes) {
        ...
        if (state.available) available.push(time);
        else {
        ...
        if (service.type === 'group') {
          ...
          slotDetails[key][time] = { occupied: state.occupied, capacity: state.capacity, remaining: state.remaining, available: state.available };
        }
      }
      if (available.length > 0) freeSlots[key] = available;
    }

    res.json({ success: true, service: { id: service.id, type: service.type, capacity: service.capacity }, allSlots, freeSlots, busySlots: busyMap, slotDetails });
  } catch (error) {
...
const blockDate = async (req, res) => {
  try {
    ...
    await new Promise((resolve, reject) => {
      db.db.run('INSERT OR REPLACE INTO schedule_exceptions (date, reason) VALUES (?, ?)', [date, reason || ''], (err) => { if (err) reject(err); else resolve(); });
    });
...
const unblockDate = async (req, res) => {
  try {
    ...
    await new Promise((resolve, reject) => {
      db.db.run('DELETE FROM schedule_exceptions WHERE date = ?', [date], (err) => { if (err) reject(err); else resolve(); });
    });
